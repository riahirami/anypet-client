...
export const getNotificationMessage = (notification: any): string => {
  ...
  if (notification.type === "App\\Notifications\\AdStatusUpdated") {
    return "The status of your ad " + notification?.data?.title + " has been updated and " + statusToString(notification?.data?.status);
  }
  ...
  if (notification.type === "App\\Notifications\\RespondOnReservationNotification") {
    return "The status of your reservantion for the ad " + notification?.data?.ad + " has been updated to " + statusToString(notification?.data?.status);
  }
  return "";
};
